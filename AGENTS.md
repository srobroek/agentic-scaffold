...
| A failure that looks like a mystery | `docs/gotchas.md` |
...
Record a failure found by running something in `docs/gotchas.md`, with the
cause.

## Work

Tracked in beads, prefix `psc`. `bd ready` lists claimable work. Run `bd prime`
for the beads workflow.
