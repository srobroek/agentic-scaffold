...
# The caller is the one file the host layer cannot ship: it names the reusable workflows
# the lang/* layers contributed, and which ones exist depends on what rendered. So it is
# generated, and these check the generator against a real rendered tree rather than a
# fixture, because the paths it wires have to be the paths the layers wrote.


@pytest.fixture
def caller(tmp_path: Path) -> Path:
    """host/github over lang/rust, which is the render order `after: [lang/*]` declares."""
    dest = tmp_path / "c"
    dest.mkdir()
    assert render("lang/rust", dest, "crate_kind: lib\n").returncode == 0
    result = render("host/github", dest, ANSWERS)
    assert result.returncode == 0, result.stderr
    return dest


def sync(dest: Path, *args: str) -> subprocess.CompletedProcess[str]:
    """The shipped copy of the generator, which is the one `just ci-sync` runs."""
    return subprocess.run(
        [sys.executable, str(dest / "scripts" / "gen_caller.py"), str(dest), *args],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_the_render_writes_the_caller(caller: Path) -> None:
    """The copier task, not a second command: a rendered repository whose gate names a job
    no file calls has a required check that never reports.
    """
    body = (caller / ".github" / "workflows" / "ci.yml").read_text()
    assert body.startswith("# Generated by scripts/gen_caller.py")
    assert (caller / "scripts" / "gen_caller.py").is_file()


def test_the_caller_wires_each_discovered_language(caller: Path) -> None:
    """One lint and one test job per contributed workflow, each gated on its own filter.

    The condition is the point of the `changes` job: a docs-only pull request skips the
    language jobs, and the gate counts a skip as a pass.
    """
    spec = workflow(caller, "ci")
    jobs = spec["jobs"]

    for name in ("lint-rust", "test-rust"):
        assert jobs[name]["needs"] == "changes"
        assert jobs[name]["if"] == "contains(fromJSON(needs.changes.outputs.changes), 'rust')"
        assert jobs[name]["uses"] == f"./.github/workflows/wc-{name.split('-')[0]}-rust.yml"

    # The filter spec travels as a YAML string, so it is parsed rather than matched.
    filters = yaml.safe_load(jobs["changes"]["with"]["filters"])
    assert "**/*.rs" in filters["rust"]
    # A change to the workflow, or to the caller that wires it, has to run the job too.
    assert ".github/workflows/ci.yml" in filters["rust"]

    # Neither takes a language input, and neither waits for the filter.
    for name in ("quality", "security"):
        assert "needs" not in jobs[name]
        assert "if" not in jobs[name]
    assert "with" not in jobs["quality"]


def test_the_caller_grants_the_scans_their_upload_scope(caller: Path) -> None:
    """A called workflow cannot hold more than its caller granted, so without this the run
    fails at the SARIF upload rather than at the scan. Naming permissions replaces the
    default set, so `contents: read` has to be repeated.
    """
    permissions = workflow(caller, "ci")["jobs"]["security"]["permissions"]
    assert permissions == {"contents": "read", "security-events": "write"}


def test_the_generated_gate_needs_every_other_job(caller: Path) -> None:
    """`gate` is the only required check, so a job it does not need is a job branch
    protection never sees.
    """
    spec = workflow(caller, "ci")
    gate = spec["jobs"]["gate"]

    assert set(gate["needs"]) == set(spec["jobs"]) - {"gate"}
    assert gate["with"]["needs"] == "${{ toJSON(needs) }}"
    # Without always() the gate is skipped whenever anything it needs was skipped, and a
    # required check that never reports blocks the pull request forever.
    assert gate["if"] == "always()"


def test_the_caller_filters_at_job_level_only(caller: Path) -> None:
    """The rule the whole `changes` job exists for. A workflow gated at `on:` does not run
    for an unrelated change, and the gate would stay pending.
    """
    spec = workflow(caller, "ci")
    assert sorted(spec["on"]) == ["pull_request", "push"]
    for trigger, config in spec["on"].items():
        if isinstance(config, dict):
            assert "paths" not in config, f"ci.yml filters paths under {trigger}"
            assert "paths-ignore" not in config
    # A branch with an open pull request would otherwise run the whole pipeline twice.
    assert spec["on"]["push"]["branches"] == ["main"]


def test_a_second_sync_writes_the_same_bytes(caller: Path) -> None:
    """The file is committed, so a non-deterministic generator makes every run a diff."""
    path = caller / ".github" / "workflows" / "ci.yml"
    first = path.read_bytes()

    assert sync(caller).returncode == 0

    assert path.read_bytes() == first


def test_a_hand_owned_caller_is_refused(caller: Path) -> None:
    """Somebody wrote their own, which is the escape hatch for a job the generator cannot
    derive. Overwriting it would delete the only copy.
    """
    path = caller / ".github" / "workflows" / "ci.yml"
    mine = "name: CI\n\non:\n  pull_request:\n\njobs: {}\n"
    ...
    assert "hand-owned" in result.stderr
    assert path.read_text() == mine

    # The render has to survive the same case: a copier task that fails aborts everything.
    kept = sync(caller, "--keep-hand-owned")
    assert kept.returncode == 0
    assert path.read_text() == mine


def test_a_language_with_no_filter_entry_is_refused(caller: Path) -> None:
    """An unfiltered job runs on every change and a guessed filter skips a change it should
    have tested, so the language is named instead.
    """
    (caller / ".github" / "workflows" / "wc-lint-zig.yml").write_text("on:\n  workflow_call:\n")

    result = sync(caller)

    assert result.returncode == 1
    assert "zig" in result.stderr
    assert "gen_caller.py" in result.stderr


def test_the_generated_caller_passes_actionlint(caller: Path) -> None:
    """It calls local reusable workflows, so actionlint resolves each one and checks the
    inputs against what that workflow declares. Skipped rather than failed when the tool is
    absent: it is installed through mise in CI.
    """
    if shutil.which("actionlint") is None:
        pytest.skip("actionlint absent from PATH")

    result = subprocess.run(
        ["actionlint", str(caller / ".github" / "workflows" / "ci.yml")],
        capture_output=True,
        text=True,
        check=False,
    )
